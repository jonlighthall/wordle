...
def calculate_frequency_score(word: str, word_list: List[str]) -> Tuple[int, float]:
    ...
    for i in range(word_length):
        if freq[i]:  # Check if position has any letters
            ...
            print(
                f"Position {i+1}: '{most_common_letter}' (appears {freq[i][most_common_letter]} times)"
            )
        else:
...
def find_best_frequency_words(
    ...
    calculate_entropy_upfront: bool = False,
    unique_letters_only: bool = True,
    wordle_appropriate_only: bool = True,
    ...
    if wordle_appropriate_only:
        ...
        print(
            f"Filtered to {filtered_count} Wordle-appropriate words from {original_count} total words"
        )
    ...
    if unique_letters_only:
        ...
        if wordle_appropriate_only:
            print(
                f"Further filtered to {filtered_count} words with unique letters (isograms)"
            )
        else:
            print(
                f"Filtered to {filtered_count} words with unique letters (isograms) from {original_count} total words"
            )

    print(
        f"📊 Calculating {analysis_type} frequency scores for {len(word_list)} words..."
    )
    ...
    print(
        f"   Letter frequency analysis complete. Now scoring {len(word_list)} words..."
    )
...
def main():
    ...
    word_list = load_word_list_with_fallback(
        "words_alpha5.txt", ["words_alpha5_100.txt"]
    )
    if not word_list:
        return
    ...
    top_words = find_best_frequency_words(
        ...
        top_n=20,
        find_lowest=False,
        unique_letters_only=False,
        wordle_appropriate_only=False,
    )

    print(
        f"{'Rank':<4} {'Word':<8} {'Freq Score':<10} {'Likelihood':<12} {'Entropy':<10}"
    )
    print(f"{'-'*55}")

    for i, (word, freq_score, likelihood_score, entropy) in enumerate(top_words, 1):
        print(
            f"{i:<4} {word:<8} {freq_score:<10} {likelihood_score:<12.4f} {entropy:<10.4f}"
        )
    ...
    print(f"{'='*80}")
    print(
        f"Highest frequency word: {top_words[0][0]} (score: {top_words[0][1]}, entropy: {top_words[0][3]:.4f})"
    )
    print(
        f"Lowest frequency word:  {bottom_words[0][0]} (score: {bottom_words[0][1]}, entropy: {bottom_words[0][3]:.4f})"
    )
